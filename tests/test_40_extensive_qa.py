...
class TestQA35BuildCommandSentinels:
    ...
    def _build(self, prompt, agent="test", template="echo {prompt}"):
        ...
        with patch.object(settings, "agent_commands", {agent: template}):
            task = SimpleNamespace(prompt=prompt, agent=agent, project_dir="/tmp", model=None, parent_task_id=None)
    ...
    def test_unknown_agent_returns_echo(self):
        from app.core.runner import AgentRunner
        r = AgentRunner()
        task = SimpleNamespace(prompt="test", agent="nonexistent", project_dir="/tmp", model=None, parent_task_id=None)
